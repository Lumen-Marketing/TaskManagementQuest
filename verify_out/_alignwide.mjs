...
const r = await page.evaluate(() => {
  const eb = document.querySelector('.page-eyebrow'); const t = document.querySelector('.page-title');
  const rb = (el)=>{const g=document.createRange();g.selectNodeContents(el);return +g.getBoundingClientRect().left.toFixed(1);};
  return { eyebrowGlyph: eb?rb(eb):null, titleGlyph: rb(t), card: +document.querySelector('.task-group').getBoundingClientRect().left.toFixed(1) };
});
console.log(JSON.stringify(r));
await page.screenshot({ path: 'verify_out/align_wide.png', clip: { x: 255, y: 120, width: 340, height: 240 } });
await browser.close();
